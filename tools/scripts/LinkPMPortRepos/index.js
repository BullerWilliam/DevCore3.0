...
const pmLink = /^(github:PenguinMod|git\+https:\/\/github\.com\/PenguinMod)(-Dev|-Port)?\/(PenguinMod|scratch)-([\w-]+)(.git)?(#[\w-]+|)$/i
...
// TODO: There's currently some weird "infinite" recursion where the same paths are being
// linked over and over, not sure why
const ittr = async entry => {
    ...
    const dependencies = Object.entries(Object.assign({}, packJson.dependencies, packJson.devDependencies))
        .filter(package => package[1].startsWith('github:PenguinMod')
            || package[1].startsWith('git+https://github.com/PenguinMod')
            || package[1].startsWith('github:PenguinMod-Port')
            || package[1].startsWith('git+https://github.com/PenguinMod-Port')
        )
        .map(package => {
