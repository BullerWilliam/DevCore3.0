...
<style>
  :global(:root) {
    --devcore-topbar: #00c3ff;
    --devcore-topbar-text: #ffffff;
    --devcore-accent: #ff9f1a;
...
<main aria-hidden={modalVisible} class:is-not-safari={!isSafari}>
  <Section accent="var(--devcore-topbar)">
    <div>
      <h1>{APP_NAME}</h1>
      <p class="brand-note">DevCore theming is synced from the home site.</p>
      {#if version}
        <p class="version">
          {version}
          {#if isStandalone}
